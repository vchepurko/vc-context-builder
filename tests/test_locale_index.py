...
class BuildLocaleIndexTests(unittest.TestCase):
    ...
    def test_single_language_single_namespace(self) -> None:
        _write(os.path.join(self.root, "locales", "uk", "common.json"), {
            "hello": "Привіт",
            "bye": "Бувай",
        })
        index = locale_index.build_locale_index(self.root)
    ...
    def test_two_languages_full_parity(self) -> None:
        _write(os.path.join(self.root, "locales", "uk", "common.json"),
               {"hello": "Привіт"})
        _write(os.path.join(self.root, "locales", "en", "common.json"),
               {"hello": "Hello"})
        index = locale_index.build_locale_index(self.root)
        entry = index["hello"]
    ...
    def test_namespace_owners_drive_missing_calculation(self) -> None:
        ...
        _write(os.path.join(self.root, "locales", "uk", "admin.json"),
               {"role_label": "Роль"})
    ...
    def test_custom_locales_path(self) -> None:
        _write(os.path.join(self.root, "i18n", "uk", "common.json"),
               {"hi": "привіт"})
...
class WriteLocaleIndexTests(unittest.TestCase):
    ...
    def test_persists_sorted(self) -> None:
        index = {
            "z_key": {"namespace": "common", "languages": ["uk"],
                      "values": {"uk": "z"}, "missing": []},
            "a_key": {"namespace": "common", "languages": ["uk"],
                      "values": {"uk": "a"}, "missing": []},
        }
...
class QueryEngineLocaleTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        _write_json(os.path.join(self.root, "agent_locale_keys.json"), {
            "staff_detail": {
                "namespace": "admin",
                "languages": ["en", "uk"],
    ...
    def test_list_filters_by_namespace(self) -> None:
        engine = QueryEngine(self.root)
        self.assertEqual(engine.list_locale_keys(namespace="admin"),
                         ["staff_detail"])
        self.assertEqual(engine.list_locale_keys(namespace="common"),
                         ["common_btn_back"])
    ...
    def test_missing_artifact_degrades_gracefully(self) -> None:
        ...
        _write_json(os.path.join(empty_root, "agent_root.json"), {
            "project_root": empty_root, "modules": ["."], "roles": {},
        })
