    ...
    return [
        ...
        },
        {
            ...
            "description": (
                ...
                "names present in ``agent_di_index.json``, returns "
                ...
                "**Live scan fallback:** used for names not in the DI index (plain "
                "functions, dotted paths, Python callables). Regex-based walk — "
                "always current but slower on large trees.\n\n"
                "**TypeScript limitation:** the live scan finds plain string "
                "occurrences only — it does NOT understand TS types or method "
                "resolution. For TS function calls that are not Angular DI "
                "injections this tool returns empty. Use ``find_in_file`` with "
                "the call expression as the pattern instead.\n\n"
